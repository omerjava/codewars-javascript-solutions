...
//console.log(concat("a",4));
function mix(s1, s2) {
    const str1=s1.split("").filter(val => "abcdefghijklmnopqrstuvwxyz".includes(val))
    .filter((val,i,arr) => arr.indexOf(val)!==arr.lastIndexOf(val));
    const str2=s2.split("").filter(val => "abcdefghijklmnopqrstuvwxyz".includes(val))
    .filter((val,i,arr) => arr.indexOf(val)!==arr.lastIndexOf(val));
    //console.log(str1);
   // console.log(str2);
    const obj1={};
    ...
    for(let element in obj1){
        if(obj1[element]>obj2[element]||(obj1[element]&&!obj2[element])){
            let x=concat(element,obj1[element]);
            let y="1:"+x;
            result.push(y);
        }
        else if(obj1[element]<obj2[element]){
            let x=concat(element,obj2[element]);
            let y="2:"+x;
            result.push(y);
        }
        else{
    ...
    for(let j=strLen;j>1;j--){
        const subset=[];
        for(let i=0;i<result.length;i++){  
            if(result[i].length===j){
                subset.push(result[i]);
            }     
        }
        lastResult.push(subset.sort());
     }
    // console.log(obj1);
